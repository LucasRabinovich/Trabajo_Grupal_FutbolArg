...
const cargarEscudos = async () => {
  try {
    ...
    console.error('Error al cargar escudos de Mi Equipo:', error)
...
const filtrarPartidosFavoritos = () => {
  partidosFiltrados.value = todosLosPartidos.filter((partido) => {
    return partido.local === favorito.value || partido.visitante === favorito.value
  })
}
